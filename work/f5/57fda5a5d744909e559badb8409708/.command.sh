...
do
    ...
    do
        find /data-weedomics-1/align_fastq/test/reads -name '*.bam' | grep "paired-" | grep "${name}" | head -n1 >> bamlist-${g}.txt
    done
done

echo 'Pileup in parallel.'
parallel         -j 32         sh /data-weedomics-1/align_fastq/modules/../scripts/pileup.sh             {}             /data-weedomics-1/align_fastq/test/ref/Lolium_rigidum_genome.fasta             20             20         ::: $(ls bamlist-*.txt)
